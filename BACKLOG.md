...
- [ ] Remaining credit_card gaps (evadex replay 2026-07-04, post #365-#367; 63/100 previously-bypassing variants now detected) — `noise_embedded` and `base64_partial` now detected (7f8ab32); still bypassing: zero-padding (`right_pad_zeros`/`left_pad_zeros`, ~10), nested/partial base64 chains (`base64_no_padding`/`base64_double`/`url_of_base64`/`hex_of_base64`, ~9), residual `homoglyph_substitution` (4), `barcode_split` (2), and a couple of `morse_slash_sep`/`morse_no_sep` stragglers.
